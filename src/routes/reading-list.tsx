    ...
    <div className="min-h-screen bg-background pb-12">
      ...
      <div className="mx-auto max-w-6xl px-4 pt-24">
        <div className="flex items-center gap-4 mb-12">
          ...
          <div>
            <h1 className="text-4xl font-display font-semibold text-gradient-brand">Reading List</h1>
            <p className="text-muted-foreground mt-2">Books you want to read ({wantToRead.length})</p>
          </div>
        </div>

        {wantToRead.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 text-center glass rounded-3xl mt-12">
             <Bookmark className="h-8 w-8 text-brand-3 mb-4 opacity-50" />
             <div className="text-lg text-muted-foreground">Your reading list is empty</div>
          </div>
        ) : (
